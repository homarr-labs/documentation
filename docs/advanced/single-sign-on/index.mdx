...
<Tabs>
  ...
  <TabItem value="oidc" label="OIDC provider">
    ...
    | ``AUTH_OIDC_TOKEN_ENDPOINT_AUTH_METHOD`` | Override method for the token endpoint authentication. Supported values are `client_secret_basic`, `client_secret_post`, and `none`. | client_secret_basic |
